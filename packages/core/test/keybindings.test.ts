...
test("actionForKey matches the default chords exactly", () => {
  ...
  // pause moved off shift+escape (terminals can't encode it) onto ctrl+space, which transmits
  // everywhere (NUL byte) and is single-handed
  expect(actionForKey({ name: "space", ctrl: true }, map)).toBe("pause.open")
