...
function nbFile(dir: string) {
  ...
  fs.writeFileSync(
    p,
    JSON.stringify({ cells: [{ cell_type: "code", source: ["print(1)"], metadata: {}, outputs: [], execution_count: null }], metadata: {}, nbformat: 4, nbformat_minor: 5 }),
  )
  return p
}
...
test("edit replaces a cell's source and clears outputs", async () => {
  ...
  nbFile(dir)
  const res = await notebookEditTool.execute({ path: "nb.ipynb", action: "edit", cell: 0, source: "print(2)" }, ctx(dir))
...
test("insert adds a markdown cell; delete removes a cell", async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nb-"))
  nbFile(dir)
  await notebookEditTool.execute({ path: "nb.ipynb", action: "insert", cell: 0, cell_type: "markdown", source: "# Title" }, ctx(dir))
